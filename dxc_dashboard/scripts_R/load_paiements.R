# ============================================================================
# SCRIPT 4/4 — tb_ftth_postpaid_paiements
# Chargement paiements FTTH Postpaid depuis CSV local → PostgreSQL
# Logique : INSERT avec ON CONFLICT DO NOTHING (ajout incrémental)
# Clé unique : data_jour + co_id
# ============================================================================

library(DBI); library(RPostgreSQL); library(readr)
library(dplyr); library(stringr); library(lubridate)

BASE_DATA <- Sys.getenv("BASE_DATA")
if (BASE_DATA == "") BASE_DATA <- getwd()

PATH_CSV <- file.path(BASE_DATA, "paiements")
LOG_DIR  <- file.path(BASE_DATA, "logs")
if (!dir.exists(LOG_DIR)) dir.create(LOG_DIR, recursive = TRUE)
LOG_FILE <- file.path(LOG_DIR, paste0("load_paiements_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".txt"))
SEPARATOR <- strrep("=", 70); debut <- Sys.time()

write_log <- function(msg, level = "INFO") {
  line <- paste0("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "] [", level, "] ", msg)
  cat(line, "\n", file = LOG_FILE, append = TRUE); message(line)
}

clean_nd <- function(x) {
  val <- as.character(x) %>% str_trim() %>% str_replace_all("[^0-9]", "")
  case_when(
    str_starts(val, "225225") & str_detect(str_sub(val, 7), "^27[2-3][0-9]{7}$") ~ str_sub(val, 7),
    str_starts(val, "225")    & str_detect(str_sub(val, 4), "^27[2-3][0-9]{7}$") ~ str_sub(val, 4),
    str_starts(val, "25")     & str_detect(str_sub(val, 3), "^27[2-3][0-9]{7}$") ~ str_sub(val, 3),
    str_detect(val, "^27[2-3][0-9]{7}$") ~ val,
    TRUE ~ NA_character_
  )
}

write_log(SEPARATOR); write_log("DÉMARRAGE — tb_ftth_postpaid_paiements"); write_log(SEPARATOR)
write_log("Connexion à PostgreSQL...")
con <- tryCatch({
  dbConnect(PostgreSQL(), host=Sys.getenv("DB_HOST"), port=as.integer(Sys.getenv("DB_PORT")),
    dbname=Sys.getenv("DB_NAME"), user=Sys.getenv("DB_USER"), password=Sys.getenv("DB_PASSWORD"))
}, error = function(e) { write_log(paste("ERREUR :", e$message), "ERROR"); stop() })
write_log("✓ Connexion PostgreSQL établie")
on.exit(tryCatch(dbDisconnect(con), error = function(e) NULL), add = TRUE)

write_log("Création de la table si inexistante...")
dbExecute(con, "
  CREATE TABLE IF NOT EXISTS public.tb_ftth_postpaid_paiements (
    id                  SERIAL PRIMARY KEY,
    data_jour           TEXT,
    date_paiement       DATE,
    nd                  TEXT,
    co_id               TEXT,
    solde_tout_compte   NUMERIC,
    solde_fact_mois_m   NUMERIC,
    mnt_fact_mois_m     NUMERIC,
    solde_fact_mois_m_1 NUMERIC,
    statut_fact         TEXT,
    date_insertion      TIMESTAMP,
    cle_unique          TEXT UNIQUE
  );
  CREATE INDEX IF NOT EXISTS idx_paiements_nd     ON public.tb_ftth_postpaid_paiements(nd);
  CREATE INDEX IF NOT EXISTS idx_paiements_co_id  ON public.tb_ftth_postpaid_paiements(co_id);
  CREATE INDEX IF NOT EXISTS idx_paiements_date   ON public.tb_ftth_postpaid_paiements(date_paiement);
  CREATE INDEX IF NOT EXISTS idx_paiements_statut ON public.tb_ftth_postpaid_paiements(statut_fact);
")
write_log("✓ Table tb_ftth_postpaid_paiements vérifiée")

write_log("Recherche des fichiers CSV...")
fichiers <- list.files(PATH_CSV, pattern="\\.csv$", full.names=TRUE, ignore.case=TRUE)
if (length(fichiers) == 0) { write_log(paste("Aucun CSV dans :", PATH_CSV), "ERROR"); stop() }
write_log(paste(length(fichiers), "fichier(s) trouvé(s) :"))
for (f in fichiers) write_log(paste("  -", basename(f)))

raw <- bind_rows(lapply(fichiers, function(f) {
  write_log(paste("Lecture :", basename(f)))
  df <- read_csv(f, show_col_types=FALSE,
    col_types=cols(data_jour=col_character(), nd=col_character(), co_id=col_character(),
      solde_tout_compte=col_double(), solde_fact_mois_m=col_double(),
      mnt_fact_mois_m=col_double(), solde_fact_mois_m_1=col_double(),
      statut_fact=col_character()),
    locale=locale(encoding="UTF-8"))
  write_log(paste("  →", format(nrow(df), big.mark=" "), "lignes"))
  df
}))
write_log(paste("Total lignes brutes lues :", format(nrow(raw), big.mark=" ")))

write_log("Transformation des données...")
write_log(paste("  ND manquants avant nettoyage :", sum(is.na(raw$nd) | raw$nd == "")))

df_final <- raw %>%
  mutate(
    nd                  = clean_nd(nd),
    co_id               = as.character(co_id),
    solde_tout_compte   = as.numeric(solde_tout_compte),
    solde_fact_mois_m   = as.numeric(solde_fact_mois_m),
    mnt_fact_mois_m     = as.numeric(mnt_fact_mois_m),
    solde_fact_mois_m_1 = as.numeric(solde_fact_mois_m_1),
    statut_fact         = as.character(statut_fact),
    data_jour           = format(ymd(as.character(data_jour), quiet=TRUE), "%Y-%m-%d"),
    date_paiement       = ymd(as.character(data_jour), quiet=TRUE),
    date_insertion      = Sys.time(),
    cle_unique          = paste(data_jour, co_id, sep="_")
  ) %>%
  select(data_jour, date_paiement, nd, co_id, solde_tout_compte, solde_fact_mois_m,
         mnt_fact_mois_m, solde_fact_mois_m_1, statut_fact, date_insertion, cle_unique) %>%
  filter(!is.na(co_id), !is.na(date_paiement))

write_log(paste("  Lignes filtrées :", nrow(raw) - nrow(df_final)))
write_log(paste("  Lignes après transformation :", format(nrow(df_final), big.mark=" ")))
statuts <- df_final %>% count(statut_fact)
write_log("  Répartition statut_fact :")
for (i in 1:nrow(statuts)) write_log(paste("    -", statuts$statut_fact[i], ":", format(statuts$n[i], big.mark=" ")))

# ============================================================================
# INSERTION INCRÉMENTALE — ON CONFLICT DO NOTHING
# ============================================================================
write_log("Insertion incrémentale dans tb_ftth_postpaid_paiements...")
start_insert <- Sys.time()

tryCatch({
  dbWriteTable(con, name="temp_paiements", value=df_final,
               temporary=TRUE, overwrite=TRUE, row.names=FALSE)

  n_inserted <- dbExecute(con, "
    INSERT INTO public.tb_ftth_postpaid_paiements
      (data_jour, date_paiement, nd, co_id, solde_tout_compte, solde_fact_mois_m,
       mnt_fact_mois_m, solde_fact_mois_m_1, statut_fact, date_insertion, cle_unique)
    SELECT data_jour, date_paiement, nd, co_id, solde_tout_compte, solde_fact_mois_m,
       mnt_fact_mois_m, solde_fact_mois_m_1, statut_fact, date_insertion, cle_unique
    FROM temp_paiements
    ON CONFLICT (cle_unique) DO NOTHING
  ")

  duree <- round(as.numeric(difftime(Sys.time(), start_insert, units="secs")), 2)
  write_log(paste("✓ Insertion terminée en", duree, "secondes"))
  write_log(paste("  Nouvelles lignes insérées :", format(n_inserted, big.mark=" ")))
  write_log(paste("  Doublons ignorés          :", format(nrow(df_final) - n_inserted, big.mark=" ")))

}, error = function(e) {
  write_log(paste("ERREUR insertion :", e$message), "ERROR"); stop()
})

# ============================================================================
# VÉRIFICATION FINALE
# ============================================================================
stats <- dbGetQuery(con, "
  SELECT COUNT(*) AS total_lignes, COUNT(DISTINCT nd) AS nd_uniques,
    COUNT(DISTINCT co_id) AS contrats_uniques,
    COUNT(DISTINCT data_jour) AS jours_distincts,
    MIN(date_paiement) AS premiere_date, MAX(date_paiement) AS derniere_date,
    SUM(mnt_fact_mois_m) AS montant_total,
    SUM(CASE WHEN statut_fact='paye'        THEN 1 ELSE 0 END) AS nb_payes,
    SUM(CASE WHEN statut_fact='non_paye'    THEN 1 ELSE 0 END) AS nb_non_payes,
    SUM(CASE WHEN statut_fact='indetermine' THEN 1 ELSE 0 END) AS nb_indetermines
  FROM public.tb_ftth_postpaid_paiements
")

write_log(""); write_log(SEPARATOR); write_log("STATISTIQUES FINALES"); write_log(SEPARATOR)
write_log(paste("Total lignes en base :", format(stats$total_lignes, big.mark=" ")))
write_log(paste("ND uniques           :", format(stats$nd_uniques, big.mark=" ")))
write_log(paste("Contrats uniques     :", format(stats$contrats_uniques, big.mark=" ")))
write_log(paste("Jours distincts      :", stats$jours_distincts))
write_log(paste("Première date        :", format(stats$premiere_date, "%d/%m/%Y")))
write_log(paste("Dernière date        :", format(stats$derniere_date, "%d/%m/%Y")))
write_log(paste("Montant total fact.  :", format(stats$montant_total, big.mark=" ", scientific=FALSE), "FCFA"))
write_log(paste("Payés                :", format(stats$nb_payes, big.mark=" ")))
write_log(paste("Non payés            :", format(stats$nb_non_payes, big.mark=" ")))
write_log(paste("Indéterminés         :", format(stats$nb_indetermines, big.mark=" ")))
write_log(paste("Durée totale         :", round(as.numeric(difftime(Sys.time(), debut, units="secs")), 2), "secondes"))
write_log(""); write_log(SEPARATOR)
write_log("PROCESSUS TERMINÉ AVEC SUCCÈS — tb_ftth_postpaid_paiements")
write_log(SEPARATOR); write_log(paste("Log :", LOG_FILE))

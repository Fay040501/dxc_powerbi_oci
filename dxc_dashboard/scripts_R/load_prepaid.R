# ============================================================================
# SCRIPT 1/4 — tb_ftth_prepaid
# Chargement snapshot FTTH Prepaid depuis CSV local → PostgreSQL
# Logique : INSERT avec ON CONFLICT DO NOTHING (ajout incrémental)
# Clé unique : nd + date_expiration + date_id
# ============================================================================
...
PATH_CSV <- file.path(BASE_DATA, "prepaid")
LOG_DIR  <- file.path(BASE_DATA, "logs")
if (!dir.exists(LOG_DIR)) dir.create(LOG_DIR, recursive = TRUE)
LOG_FILE <- file.path(LOG_DIR, paste0("load_prepaid_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".txt"))
...
write_log(SEPARATOR); write_log("DÉMARRAGE — tb_ftth_prepaid"); write_log(SEPARATOR)
...
dbExecute(con, "
  CREATE TABLE IF NOT EXISTS public.tb_ftth_prepaid (
  ...
  CREATE INDEX IF NOT EXISTS idx_prepaid_nd      ON public.tb_ftth_prepaid(nd);
  CREATE INDEX IF NOT EXISTS idx_prepaid_date_id ON public.tb_ftth_prepaid(date_id);
  CREATE INDEX IF NOT EXISTS idx_prepaid_etat    ON public.tb_ftth_prepaid(etat);
")
write_log("✓ Table tb_ftth_prepaid vérifiée")
...
raw <- bind_rows(lapply(fichiers, function(f) {
  ...
  df <- read_csv(f, show_col_types=FALSE, col_types=cols(nd=col_character(), date_expiration=col_character(), etat=col_character(), date_id=col_date()), locale=locale(encoding="UTF-8"))
...
df_final <- raw %>%
  mutate(
    nd              = clean_nd(nd),
    date_expiration = as.character(date_expiration),
    etat            = as.character(etat),
    date_id         = as.Date(date_id),
    date_insertion  = Sys.time(),
    cle_unique      = paste(nd, date_expiration, date_id, sep="_")
  ) %>%
...
# Seules les nouvelles lignes sont insérées, les existantes ignorées
# ============================================================================
write_log("Insertion incrémentale dans tb_ftth_prepaid...")
start_insert <- Sys.time()

tryCatch({
  dbWriteTable(con, name="temp_prepaid", value=df_final,
               temporary=TRUE, overwrite=TRUE, row.names=FALSE)

  n_inserted <- dbExecute(con, "
    INSERT INTO public.tb_ftth_prepaid
      (nd, date_expiration, etat, date_id, date_insertion, cle_unique)
    SELECT nd, date_expiration, etat, date_id, date_insertion, cle_unique
    FROM temp_prepaid
...
write_log("PROCESSUS TERMINÉ AVEC SUCCÈS — tb_ftth_prepaid")
